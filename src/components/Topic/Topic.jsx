  ...
  render() {
    ...
    return (
      <li className={topic}>
        ...
        />
        <Link to={uri} className={isSelected}>
          {' '}
          {this.props.name}{' '}
        </Link>
...
function getBullet(x) {
  switch (x) {
    case 'Closed':
      return { className: 'oi oi-caret-right cursor-pointer', title: 'Déplier' }
    case 'Opened':
      return {
        className: 'oi oi-caret-bottom cursor-pointer',
        title: 'Replier'
      }
    default:
      return { className: 'oi leaf', title: '' }
  }
}
